...
def contains_superset(ranges):
    ranges_0_is_a_subset = ranges[0][0] >= ranges[1][0] and ranges[0][1] <= ranges[1][1]
    if not ranges_0_is_a_subset:
        ranges_1_is_a_subset = ranges[1][0] >= ranges[0][0] and ranges[1][1] <= ranges[0][1]
        if not ranges_1_is_a_subset:
            return False
    return True
